...
특히 `ApiResponse`, `GlobalExceptionHandler`, `DomainException` 처럼 응답과 예외 처리를 위해 여러 클래스를 나누는 부분이 처음에는 복잡하게 느껴졌다. 하지만 각각 **응답 형식 정의, 예외 처리, 도메인 예외포현**이라는 서로 다른 역할을 담당한다는 것을 알게 되었다.
